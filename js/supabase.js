    ...
    if (SUPABASE_URL && SUPABASE_ANON_KEY) {
        try {
            ...
            supabaseClient.auth.onAuthStateChange((event, session) => {
                if (event === 'SIGNED_IN') console.log('🔐 Auth: تم تسجيل الدخول');
                else if (event === 'SIGNED_OUT') console.log('🔐 Auth: تم تسجيل الخروج');
                else if (event === 'TOKEN_REFRESHED') console.log('🔐 Auth: تم تجديد التوكن');
            });
...
async function loadAllFromSupabase() {
    ...
    const authed = await isSupabaseAuthenticated();
    if (!authed) {
        console.log('☁️ Supabase: التحميل من السحابة يتطلب تسجيل الدخول — تم التجاهل');
        return false;
    }
...
// التحقق من حالة المصادقة (Auth session)
